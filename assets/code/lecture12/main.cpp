...
static const std::map<std::string,
    ...
    algorithms = {{"soundex", soundex}, {"soundexRanges", soundexRanges}};
...
void showSoundexCodes(const std::vector<std::string>& names,
                      const std::string& algo, Soundex soundex) {
...
template <typename Soundex>
void timeSoundex(const std::vector<std::string>& names, const std::string& algo,
                 Soundex soundex, size_t rounds = 8) {
  ...
  std::cout << "Ran algorithm '" << algo << "' " << num_rounds << " times"
            << std::endl;
