...
Photo::Photo(const Photo& other)
  ...
  log() << "  ↪ copying " << width << "x" << height << " pixels..."
        << std::endl;
...
Photo& Photo::operator=(const Photo& other) {
  ...
  log() << "  ↪ cleaning up " << width << "x" << height << " pixels..."
        << std::endl;
...
void time_func(Func f, size_t invocations) {
  ...
  for (size_t i = 0; i < invocations; ++i) f();
...
int main() {
  ...
  std::cout << "Timing " << invocations << " attempts to move/copy a Photo..."
            << std::endl;
