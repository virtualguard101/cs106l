...
void test_min_basic() {
  ...
  out() << "min_basic<std::string>(\"Jacob\", \"Fabio\"): " << result
        << string_min << end;
...
void test_min_ref() {
  ...
  out() << "min_ref<std::string>(\"Arwen\", \"Aragorn\"): " << result
        << string_min << end;
}
...
void test_find() {
  ...
  // then most likely the function is in `namespace std` too!". Since
  // `v.begin()` and `v.end()` are `std::vector<int>::iterator`, it thinks that
  // `find` is actually `std::find` (not the `find` we defined above).
  //
  ...
  out() << "find<std::vector<int>::iterator, int>(v.begin(), v.end(), 3): "
        << result << pit(v, it1) << end;
  ...
  std::unordered_set<std::string> us{"hello", "there", "welcome", "to",
                                     "cs106l!"};
...
/* Concepts are a way to constrain what types a template parameter can
 * represent. By default, writing `typename T` allows `T` to be instantiated
 * with any type! That doesn't mean, however, that after instantiation, a
 * specific choice of `T` won't necessarily cause a compiler error.
 *
 ...
 * to compare the two `std::ifstream` using `operator<`, we'd get a compiler
 * error!
 *
 * Concepts are a way to constrain these types, so that `min` for example
 * doesn't even get instantiated in the first place if `T` doesn't have an
 * `operator<`. This leads to all around better error messages, faster compile
 * times, etc. */

template <typename T>
concept Comparable = requires(T a, T b) {
  // For T to model Comparable, a < b must a valid expression at compile time
  // Furthermore, we require that the result type of `a < b` models
  // `std::convertible_to<bool>`.
  //
  // Aside: `std::convertible_to<From, To>` is actually a concept that takes in
  // two type parameters and says whether `From` can be converted to `To`.
  // Notice, however, that in the constraint below, `From` is implicitly
  // replaced with the type of `a < b`, e.g. `decltype(a < b)`.
...
void test_min_var() {
  ...
  out() << "min_var<std::string>(\"cool\", \"variadic\", \"template!\"): "
        << result << m3 << end;
...
int main() { return testing::run_tests(); }