...
static bool can_demangle() {
  ...
  if (!available)
    std::cerr << "\033[33mWarning: c++filt could not be found, so type names "
                 "will be mangled.\033[0m\n\n";

  return *available;
}

static std::string demangle(const std::string& mangled) {
  if (!can_demangle()) return mangled;
  auto demangled = execute("echo " + mangled + " | c++filt -t");
  if (!demangled.has_value()) return mangled;
...
static int run_tests() {
  ...
  for (const auto& test : tests) {
    std::cout << style_bg << std::setw(100) << std::setfill(' ') << std::left
              << test.name << style_reset << std::endl;
