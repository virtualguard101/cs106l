...
struct account_state {
  ...
  // operator overload to print nicer error messages. Try making a
  // WithdrawAccountTest fail with and without this overload to see the
  // difference in error message outputs.
 ...
 *  destroy the BankAccount object manually. Because we create the
 * BankAccountTest struct and used a test fixture (TEST_F), the constructor and
 * destructor of the BankAccountTest will automatically be called for us.
 *
 *  In this case, it doesn't save us too much work, but imagine if BankAccount
 * had hundreds of fields that had to be initialized properly. Having the
 * BankAccountTest wrapper would be super helpful.
 */
 ...
 *  Should probably have more tests here (e.g. what if we deposit a negative
 * value?)
 */
 ...
 *  -------------------------------------------------------------
 *  This is a test fixture that expects parameter inputs. This test fixture is
 * kind of like a template, where it defines a blueprint for how to make the
 * true test fixture but expects an input (the parameters) to be able to do
 * that.
 *
 *  GetParam() is how we access these inputs when the test fixture is
 * instantiated.
 */
...
/** Instantiation of Parameterized Test Fixture:
 * WithdrawAccountTest->FinalBalance
 *  ------------------------------------------------------------------------------
 ...
 *  testing::WithParamInterface<account_state> which is why we need to provide
 * the instantiation with an account_state.
 *
